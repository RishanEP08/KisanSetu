    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border p-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div><h2 className="text-xl font-semibold">{user?.name}</h2><p className="text-sm text-gray-500 capitalize">{user?.role}</p></div>
        </div>
        <div className="grid sm:grid-cols-2 gap-4 text-sm">
          ...
          <div><p className="text-gray-500">Address</p><p className="font-medium">{user?.address}</p></div>
          <div><p className="text-gray-500">District</p><p className="font-medium">{user?.district}</p></div>
          <div><p className="text-gray-500">State</p><p className="font-medium">{user?.state}</p></div>
          <div><p className="text-gray-500">Pincode</p><p className="font-medium">{user?.pincode}</p></div>
        </div>
      </div>
    </div>
  );
